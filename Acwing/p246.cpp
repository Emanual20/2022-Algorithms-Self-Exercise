...
ll Query_seggcd(int l, int r, int x, int y, int k = 1){
	ll ret = -inf;
	if (x <= l && r <= y){
		ret = tree[k].gcd;
		return ret;
...
ll Query_segsum(int l, int r, int x, int y, int k = 1){
	ll ret = 0;
	if (x <= l && r <= y){
		ret = tree[k].sum;
		return ret;
...
int main(){
    ...
    while(q--){
        ...
        if(ch == 'C'){
            ll left, right, k;
            cin >> left >> right >> k;
            Update_seg(1, n + 1, left + 1, left + 1, k);
            if(right < n) Update_seg(1, n + 1, right + 2, right + 2, -k);
        }
        else{
            ll left, right, ans = 0;
            cin >> left >> right;
            ans = Query_segsum(1, n + 1, 1, left + 1); 
